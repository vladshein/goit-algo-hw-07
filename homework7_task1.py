#Homework7 task1
'''
Напишіть алгоритм (функцію), який знаходить найбільше значення у 
двійковому дереві пошуку або в AVL-дереві. Візьміть будь-яку реалізацію
дерева з конспекту чи з іншого джерела.
'''
...
class BinarySearchTree(object):
    ...
    # Функція для знаходження суми даних всіх вузлів
    def inner_sum(self, node):
        if node:
            self.sum += node.data
            # path.append(node.data)
            self.inner_sum(node.left_child)
            self.inner_sum(node.right_child)

    # Допоміжна функція для знаходження суми
    def total_sum(self):
        self.inner_sum(self.root)
        return self.sum
...
if __name__ == '__main__':
    ...
    # Вивести значення, які залишилась
    bst.traversal()
    # Тестування функцій домашнього завдання
    print(f"Max value is {bst.get_max_value()}")
    print(f"Min value is {bst.get_min_value()}")
    print(f"Sum of data in all nodes is: {bst.total_sum()}")
